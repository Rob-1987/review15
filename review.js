...
function lastBigNumber(numbers) { //This is the start of the function where the function is given it's name lastBigNumber. The name is a parameter in coding terms.
  let lastBiggest = numbers[0]; //After reading the first line the function will then read this line where it is given a varible name of lastBiggest which equals to numbers which is set at zero. 
  for (let i = 0; i < numbers.length; i++) { //Then moving to this line it is assigning (i) to zero and say to loop through the array of numbers that will be given to the function.
    if (numbers[i] > lastBiggest) {// If the current number[i] is bigger than the last numbers then move to the next line. 
      lastBiggest = numbers[i];//This line is saying that if the last biggest number equals number[i] then move to the next line and continue with the function and give a return.
    }
  }
  return lastBiggest;// This line is saying to return the lastBiggest number that has been found.
}
